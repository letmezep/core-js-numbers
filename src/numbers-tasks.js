...
function getAverage(value1, value2) {
  const a = (value1 / 100 + value2 / 100) / 2;
  return a * 100;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cos2 = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(cos1 / cos2);
}
...
function roundToPowerOfTen(num, pow) {
  const v1 = num % 10 ** pow;
  const v2 = num - v1;
  const v3 = v2 + Math.round(v1 / 10 ** pow) * 10 ** pow;
  return v3;
}
